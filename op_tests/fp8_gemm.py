...
def mxfp8_dot_scaled_gemm(
    ...
    LHS_FMT: tl.constexpr = "e4m3",       # or "e5m2"
    RHS_FMT: tl.constexpr = "e4m3",       # or "e5m2"
...
def mxfp8_gemm(A, B, As, Bs,
              BM=128, BN=128, BK=128,
              num_warps=8, group_m=8,
              fmt="e4m3", out_dtype=torch.float16):
    M, K = A.shape
    N, K2 = B.shape
    assert K == K2
    ...
    mxfp8_dot_scaled_gemm[grid](
        ...
        GROUP_M=group_m,
        LHS_FMT=fmt, RHS_FMT=fmt,
        OUT_DTYPE=tl.float16 if out_dtype == torch.float16 else (tl.bfloat16 if out_dtype == torch.bfloat16 else tl.float32),
        num_warps=num_warps,
    )
    return C

def get_scale_element_e4m3_tcast(x, cast_mode=tcast.mxfp8e4):
    M, N = x.shape
    x_tcast = tcast.cast(x, cast_mode)
    if cast_mode == tcast.mxfp8e4:
...
def triton_fused_pack_kernel(tcast_tensor):
    ...
    scales_uint8 = scale_tensor.to(torch.uint8)
    ...
    return packed_output.reshape(-1, 3), scales_uint8
...
def test_mxfp8_dot_scaled_gemm():
    M, N, K = 128, 128, 128
    A = torch.randn((M, K), device="cuda")
    A_fp8, A_scale, A_tcast = get_scale_element_e4m3_tcast(A)

    B = torch.randn((N, K), device="cuda")
    B_fp8, B_scale, B_tcast = get_scale_element_e4m3_tcast(B)

    C = mxfp8_gemm(A_fp8, B_fp8, A_scale, B_scale, out_dtype=torch.float32)
    C_torch = A_tcast.tensor @ B_tcast.tensor.T

    # MXFP6 Tcast creation
    A_tcast_fp6 = get_scale_element_e4m3_tcast(A, cast_mode=tcast.mxfp6e2)
    B_tcast_fp6 = get_scale_element_e4m3_tcast(B, cast_mode=tcast.mxfp6e2)

    # MXFP6 packing
    A_packed, A_scaled = triton_fused_pack_kernel(A_tcast_fp6)
    B_packed, B_scaled = triton_fused_pack_kernel(B_tcast_fp6)

    # MXFP6 unpacking
    A_em_fp8 = triton_fused_unpack_gemm_kernel(A_packed, A_tcast_fp6.tensor.shape)
    B_em_fp8 = triton_fused_unpack_gemm_kernel(B_packed, B_tcast_fp6.tensor.shape)

    print(A_scaled.shape)

    # MXFP8 emulation
    C_emulated = mxfp8_gemm(A_em_fp8, B_em_fp8, A_scaled.view(M, K // 32), B_scaled.view(M, K // 32), out_dtype=torch.float32)

    print(f"Triton Tcast MXFP8 vs Torch Error: {torch.max(torch.abs(C - C_torch))}")
    print(f"Triton emualted MXFP8 Error vs Triton Tcast MXFP8: {torch.max(torch.abs(C_emulated - C_torch))}")

if __name__ == "__main__":
    test_mxfp8_dot_scaled_gemm()
