...
# matmul utilities
from aiter.ops.triton.moe_op_gemm_a4w4 import moe_gemm_a4w4, moe_gemm_torch, swizzle_scales
# numerics utilities
...
def alloc_rand(shape, device, dtype):
    if dtype.itemsize == 1:
        tmp = 2**-(torch.randn(shape, device=device, dtype=torch.bfloat16))
        return tmp
...
@pytest.mark.parametrize(
    ...
    [
        tuple(getattr(case, f.name) for f in fields(Case)) for case in [
            # Case(1, 256, 256, "mxfloat4_e2m1", "mxfloat4_e2m1", 128, 4),
            Case(32, 3072, 6144, "mxfloat4_e2m1", "mxfloat4_e2m1", 128, 4, hbm_swizzling=True),
            # Case(4096, 3072, 3072, "mxfloat4_e2m1", "mxfloat4_e2m1", 128, 4),
            # Case(16, 1024, 1024, "mxfloat4_e2m1", "mxfloat4_e2m1", 128, 4, hbm_swizzling=True),
            # Case(4096, 1024, 1024, "mxfloat4_e2m1", "mxfloat4_e2m1", 128, 4),
            # Case(16, 256, 256, "mxfloat4_e2m1", "mxfloat4_e2m1", 128, 4, hbm_swizzling=True),
            # Case(4096, 256, 256, "mxfloat4_e2m1", "mxfloat4_e2m1", 128, 4),
            # Case(1000, 704, 800, "mxfloat4_e2m1", "mxfloat4_e2m1", 8, 2),
            # Case(300, 400, 800, "mxfloat4_e2m1", "mxfloat4_e2m1", 8, 4),
        ]
    ],
)
...
def test_op(m, n, k, do_gather, do_scatter, has_y_gammas, apply_swiglu, fused_quant, 
    ...
    act_mxfp = act_dtype_str.startswith("mx")
    if act_mxfp:
    ...
    x_tri, w_tri, bias_tri, gammas = init_compute_data(m, n, k, gindx, sindx, n_expts_tot, n_expts_act,
                                                                 torch.bfloat16 if act_mxfp else act_dtype,
                                                                 torch.bfloat16,
                                                                 has_y_gammas, device=device)
    x_ref, w_ref, bias_ref = x_tri.clone(), w_tri.clone(), bias_tri.clone() 

    # downcast to mxfp, static scale fp4 is disabled
    ...
    if not act_mxfp and fused_quant:
        quant_static_scale = ref_y.abs().max().float() / 448.0 
    else:
        quant_static_scale = None
    tri_y = moe_gemm_a4w4(x_tri, w_tri, x_mx_scales_tri, w_scale_tri, x_static_scale, quant_static_scale, bias_tri, rdata, gindx, sindx, gammas, swizzle_mx_scale, out_dtype, apply_swiglu)
    if not act_mxfp and fused_quant:
