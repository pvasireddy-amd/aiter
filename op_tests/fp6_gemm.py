...
def main():
    ...
    if device == 'cpu':
        ...
        for mode in [1]:
            ...
            tritonblas.matmul_a8w8_lt(act_fp8.view(torch.int8), wt_fp8.T.view(torch.int8), _act_scale.to(torch.float32), _wt_scale.T.to(torch.float32), _blast8, selector)
            ...
            print(f"  GEMM result error (TritonBLAS FP8 vs Torch FP16 matmul): {_error4.item():.6f}")
            print(torch.allclose(_blast8, torch_result16))
